    ...
    def rco_stmt(stmt: Stmt, bindings: Dict[str, Expr]) -> Stmt:
        match stmt:
            ...
            case While(cond, body_stmts):
                ...
                cond_assignments = [Assign(x, e) for x, e in cond_bindings.items()]
                new_body_stmts = rco_stmts(body_stmts)
                return While(Begin(cond_assignments, new_cond_exp), new_body_stmts)
            case _:
    ...
    def explicate_stmt(stmt: Stmt, cont: List[cif.Stmt]) -> List[cif.Stmt]:
        match stmt:
            ...
            case While(Begin(cond_stmts, cond_exp), body_stmts):
                ...
                new_cont = [cif.If(explicate_exp(cond_exp), cif.Goto(body_label), cif.Goto(cont_label))]
                test_block = explicate_stmts(cond_stmts, new_cont)
...
def allocate_registers(program: x86.X86Program) -> x86.X86Program:
    ...
    def ul_block(label: str):
        ...
        return live_after_sets[label]

    def ul_fixpoint(labels: List[str]):
        for label in blocks:
            live_before_sets[label] = set()
        while True:
            c_live_after_sets = live_after_sets.copy()

            for label, instrs in blocks.items():
                live_after_sets[label] = ul_block(label)
            if c_live_after_sets == live_after_sets:
                break
