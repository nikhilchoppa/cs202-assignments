...
def allocate_registers(program: x86.X86Program) -> x86.X86Program:
    ...
    # assign each var a color - no two interfering vars share a color
    # prefer lower colors
    # use as few colors as possible (minimize use of memory)
    for v in all_vars:
        if v not in coloring:
            coloring[v] = 0

    # we color the interference graph to do this
    # end up with coloring: var -> color (maps var to color)
    ...
    # start with register, use stack location when you run out
    for color in coloring.values():
        if color < len(available_registers):
            color_map[color] = available_registers[color]
        else:
            color_map[color] = stack_locations_used
    ...
    stack_space = stack_locations_used * 8
